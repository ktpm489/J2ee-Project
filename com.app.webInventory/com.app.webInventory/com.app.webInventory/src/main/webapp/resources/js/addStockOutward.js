...
$('#submitStockOutward').click(function(){
	    	
	    	callSaveStockOutwardAjax();
	    });
	    
			
			$("#cancelStockOutward").on('click', function()
					{
	...
	function callSaveStockOutwardAjax()
	{
    	...
    	var stockoutward =  document.getElementById("stockoutward").value;
		...
		$('#calx tr.data').each(function(row, tr)
			...
			TableData[row]={
		        ...
		        , "unitName" : $(unitName).val()
		        , "stockId" : $(this).find(':selected').attr('stock-id')
			}
			i++;
		});
		...
		$.ajax({
			...
			success: function(data) {
				if(data.error == 1){
					...
					alert("Add new StockOutward Success");
				}
				else {
